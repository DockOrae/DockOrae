...
func Router(st *state.AppState, basePath string, static gin.HandlerFunc) *gin.Engine {
	...
	// ---------- 宿主文件 + 终端(§55/§56:面板 → Agent → 宿主) ----------
	p.GET("/files/list", H(filesList).Handler(deps))
	p.GET("/files/dirsize", H(filesDirSize).Handler(deps))
	p.POST("/files/chown", H(filesChown).Handler(deps))
	p.GET("/files/stat", H(filesStat).Handler(deps))
	p.POST("/files/touch", H(filesTouch).Handler(deps))
	p.POST("/files/mkdir", H(filesMkdir).Handler(deps))
	p.POST("/files/rename", H(filesRename).Handler(deps))
	p.POST("/files/copy", H(filesCopy).Handler(deps))
	p.POST("/files/move", H(filesMove).Handler(deps))
	p.POST("/files/remove", H(filesRemove).Handler(deps))
	p.POST("/files/chmod", H(filesChmod).Handler(deps))
	p.POST("/files/write", H(filesWrite).Handler(deps))
	p.POST("/files/upload", H(filesUpload).Handler(deps))
	p.GET("/files/download", H(filesDownload).Handler(deps))
	p.POST("/files/compress", H(filesCompress).Handler(deps))
	p.POST("/files/extract", H(filesExtract).Handler(deps))
	p.GET("/files/search", H(filesSearch).Handler(deps))
	p.GET("/files/terminal", H(hostTerminalWS).Handler(deps))
	p.GET("/files/trash/status", H(trashStatus).Handler(deps))
	p.POST("/files/trash/enable", H(trashSetEnabled).Handler(deps))
	p.GET("/files/trash/list", H(trashList).Handler(deps))
	p.POST("/files/trash/restore", H(trashRestore).Handler(deps))
	p.POST("/files/trash/delete", H(trashDelete).Handler(deps))
	p.POST("/files/trash/empty", H(trashEmpty).Handler(deps))
